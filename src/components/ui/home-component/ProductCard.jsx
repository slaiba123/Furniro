...
export default function ProductCard({ product }) {
    ...
    return (
      ...
      <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-50 flex flex-col justify-center items-center opacity-0 hover:opacity-100 transition-opacity duration-300">
          ...
          <Link href="/product-detail"><h3 className="mt-4  px-3 text-[#3A3A3A] text-xl font-bold">{product.name}</h3></Link>
        <h2 className="my-1  px-3 text-[#898989] text-sm ">{product.description}</h2>
        <p className="text-[#3A3A3A] px-3 font-bold">{product.price}</p>
        </div>
